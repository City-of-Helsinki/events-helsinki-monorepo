...
const nextBaseConfig = ({
  ...
  let nextConfig = {
    ...
    experimental: {
      ...
      // Removed turbopack config keys like 'cra' and 'turbopack' from here
    },

    // 🚨 TURBOPACK CONFIGURATION
    // Explicitly set the root directory for monorepo support.
    turbopack: {
      // Assumes next.config.mjs is two levels down from the monorepo root (where the main lockfile is)
      root: join(__dirname, '../../'),
    },
  ...
  const webpackConfig = isTurbopack
    ? {}
    : {
