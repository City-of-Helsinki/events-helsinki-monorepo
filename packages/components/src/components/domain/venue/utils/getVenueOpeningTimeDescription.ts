...
function getNextOpeningHoursFromNow(openingHours: OpeningHour[]): Time | null {
  ...
  const nextTimeToday = openingHoursToday?.times?.reduce(
    (incumbent: Time | null, time) => {
...
function formatTime(time: Time, locale: AppLanguage) {
...
function getVenueOpeningTimeDescriptor(
  ...
  return {
    isOpenNow,
    closingTime: timeMatchingNow ?? null,
