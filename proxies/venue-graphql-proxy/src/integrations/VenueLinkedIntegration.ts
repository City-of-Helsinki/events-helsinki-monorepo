...
import type { TranslationsObject, VenueDetails } from '../types';
import { translateVenue } from '../utils/utils';
...
export default class VenueLinkedIntegration<
  O
> extends VenueResolverIntegration<LinkedPlace> {
  constructor(config: Config<LinkedPlace, O>) {
    super({
  ...
  private formatter(data: LinkedPlace): Partial<VenueDetails> {
    return {
      ...
      addressLocality:
        (data?.address_locality as TranslationsObject) ?? undefined,
      position: data?.position ?? null,
      description: (data?.description as TranslationsObject) ?? null,
      name: (data?.name as TranslationsObject) ?? null,
      infoUrl: (data?.info_url as TranslationsObject) ?? null,
      streetAddress: (data?.street_address as TranslationsObject) ?? null,
      telephone: (data?.telephone as TranslationsObject) ?? null,
    };
  }
}
